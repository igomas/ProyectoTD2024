...
```
 ...
 Si seguimos visualizando el data podemos observar como tenemos dos variables que nos dan problemas , si observamos podemos ver como los datos los cuales nos podran causar problemas para un futuro analisis, tienen en comun "kg" , para detectar todos aquellos valores y asi despues modificarlos usaremos la funcion "str_detect", al tener una vez almacenados todos aquellos valores en una variable , los uniremos juntos a sus valores correspondientes en la variable Producto.
...
Si nos fijamos aun hay algunas variables que nos pueden causar problema, debido a que se realizo una mala separacion en la linea y en el momento de pasarlo al data, hay datos que no son coherente o no tienen sentido, por lo que los ajustaremos
```{r}
...
```
 ...
 Se puede observar como hay algun valor el cual ha sido mal leido y ha sido añadido, por lo que nosotros procederemos a eliminar esos valores.
```{r}
...
Podemos observar como en la variable Precio_Final tenemos valores NA, pero si nos fijamos ,esos valores se encuentran en la linea de abajo debido al ajuste que hicimos con los valores del data que contenian "kg". Asi que reemplazaremos los valores NA por los valores que hay en la fila siguiente
...
Observando las variables nos dimos cuenta de que no estan con la clase correspondiente
...
Conversión de cantidades a texto y procesamiento de productos que terminan en "100" y "200:
Primeramente convertimos los números en cadenas de texto para poder tratar con ellos. Ya que productos
como las servilletas o el pan tienen un indice de 3 cifras el qual  nosotros separamos con la funcion substr.Esto lo hacemos ya que estas cifras se nos acumulaban en la variable cantidad. El procedimiento es el siguiente:
  1.convertimos los números en cadenas de texto
  2.Filtramos los índices donde los números terminan en "100"
  3.Asignaremos a las filas de cada producto segun el indice en que localizamos donde estaban los nuevos productos completos
  
Por otro lado seguiremos el mismo procedimiento para los productoes los cuales tienen una cantidad por defecto, (en este caso el 12 de los huevos). Ademas de aplicar tambien este mismo proceso con otros productos los cuales tienen el mismo patron.
```{r}
...
```
Una vez ya hemos terminado de ajustar y modificar el data para facilitar nuestro futuro analisis , podemos ver un resumen de nuestro data
...
A continuación, mostraremos aquellas variables las cuales son de tipo numérico, las cuales usaremos a posteriori para responder las preguntas planteadas.
Como podemos observar , tenemos de variables tipo numérico el Código Postal, Teléfono, Cantidad, Precio Final y Total. Explorando en cada una de las variables podemos observar como en la variable “Cantidad” no obtendremos números demasiado grandes
...
Continuamos con las variables de tipo categórico, en nuestro caso son “Empresa”, “Direccion”, “Ciudad”, “Hora”, “Factura”, “Producto”.
...
En este caso como podemos observar nuestra única variable tipo fecha es la fecha de compra de nuestros tickets.
