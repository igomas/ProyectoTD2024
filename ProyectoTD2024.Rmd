...
#otra opcion para el precio producto
precio_producto <- unlist(strsplit(data1$Cantidad[i], " "))
        
        if ( grepl(',[0-9][0-9]$', data1$Producto[i]) & substr(data1$Producto[i],1,1) > 1 ) {
          nombre_producto <- unlist( strsplit(nombre_producto, ' ') )
          nombre_producto <- paste0( nombre_producto[1:(length(nombre_producto)-1)], collapse = ' ')
          
          precio_producto <- precio_producto[(length(precio_producto)-1)]
        
        } else {
          precio_producto <- precio_producto[length(precio_producto)]
        }
        
        precio_producto <- gsub(",",".", precio_producto)
        precio_producto <- as.numeric(precio_producto)
