...
Vamos a guardar en una variable la ruta donde se encuentra la carpeta
con todos los ficheros que utilizaremos.Usaremos el codigo list.files
para obtener una lista de archivos dentro de la carpeta de la cual solo
vamos a seleccionar los archivos que sean ".txt". Se creará una lista
donde almacenaremos los datos de cada archivo.Para poder leer el
contenido de los tickets, usaremos la funcion "read_lines" para leer
linea por linea, pondremos el encoding correspondiente. En nuestro caso
los datos que nos interesan del ticket son los que se encuentran hasta
la linea donde nos indica el precio total de la compra realizada, por lo
que pondremos que la lectura de los archivos llegue la palabra "TOTAl"
,y si no se encuentra la palabra "TOTAL" pasaremos al siguiente archivo.
Como en todos los archivos las primeras lineas contienen la misma
informacion,extraemos la información y la iremos organizando en
variables, pero en algunos casos necesitaremos hacer algun ajuste en los
valores de cada variable, por ejemplo:la forma en la que guarda los
datos en la columna Telefono, Factura y hemos indicado que la variable
cantidad en cada ticket ocupa un tamaño diferente y asi lo tenga en
cuenta para las otras variables que
...
Visualizando el data, vemos como tenemos dos variables en una , por lo
que pasaremos a separarlas, la columna Ciudad c("codigo postal","
ciudad"),la columna Fecha la cual hemos separado en fecha y hora. Como
nos podemos fijar en el data frame la variable de cantidad, no esta
ajustada de una forma que nos facilite su importacion en el caso de un
futuro analisis, por lo que deberemos ajustarla #nos fijamos en que las
lineas siguen un formato, estan las cantidades del producto y el
producto unidas, por lo que usaremos la funcion gsub para que añada una
"/", asignaremos la nueva forma a la variable del dataframe y una vez ya
hecho podemos usar la funcion separate para poder separar la columna
cantidad en c("Cantidad", "Producto"). Por ultimo, haremos un cambio en
los valores que sean numeros, ya que en R no se usan comas sino que se
usan puntos. Todos estos cambios los iremos guardando en el data
...
Si seguimos visualizando el data podemos observar como tenemos dos
variables que nos dan problemas , si observamos podemos ver como los
datos los cuales nos podran causar problemas para un futuro analisis,
tienen en comun "kg" , para detectar todos aquellos valores y asi
despues modificarlos usaremos la funcion "str_detect", al tener una vez
almacenados todos aquellos valores en una variable , los uniremos juntos
a sus valores correspondientes en la variable Producto.
...
Conversión de cantidades a texto y procesamiento de productos que
terminan en "100" y "200: Primeramente convertimos los números en
cadenas de texto para poder tratar con ellos. Ya que productos como las
servilletas o el pan tienen un indice de 3 cifras el qual nosotros
separamos con la funcion substr.Esto lo hacemos ya que estas cifras se
nos acumulaban en la variable cantidad. El procedimiento es el
siguiente: 1.convertimos los números en cadenas de texto 2.Filtramos los
índices donde los números terminan en"100" 3.Asignaremos a las filas de
cada producto segun el indice en que localizamos donde estaban los
nuevos productos completos

Por otro lado seguiremos el mismo procedimiento para los productoes los
cuales tienen una cantidad por defecto, (en este caso el 12 de los
huevos). Ademas de aplicar tambien este mismo proceso con otros
productos los cuales tienen el mismo patron.
...
Podemos observar que usando la función glimpse de la librería dplyr
proporciona una visión rápida y resumida de los datos en un dataframe,
en este caso de nuestro data1, el cual contiene los datos que queremos
analizar.

Principalmente nuestro dataset tiene un total de 1213 observaciones y 12
variables. Tras un proceso de limpieza y preprocesamiento de los datos,
el dataset data1 contiene datos los cuales nos interesan para hacer
nuestro análisis posterior. Tras esta depuración de los datos, las
variables que nos convienen están mostradas en la Tabla 1.

## Variables tipo numericas

A continuación, mostraremos aquellas variables las cuales son de tipo
numérico, las cuales usaremos a posteriori para responder las preguntas
planteadas. Como podemos observar , tenemos de variables tipo numérico
el Código Postal, Teléfono, Cantidad, Precio Final y Total. Explorando
en cada una de las variables podemos observar como en la variable
"Cantidad" no obtendremos números demasiado grandes
...
Continuamos con las variables de tipo categórico, en nuestro caso son
"Empresa", "Direccion", "Ciudad", "Hora", "Factura", "Producto".
...
# Crear un gráfico de barras del número de productos por factura
barplot(num_productos_por_ticket$num_productos,
        main = "Número de productos por factura",
        xlab = "Factura",
        ylab = "Número de productos",
        col = "lightblue",
        ylim = c(0, max(num_productos_por_ticekt$num_productos) * 1.1))

# Añadir una línea horizontal para indicar el promedio
